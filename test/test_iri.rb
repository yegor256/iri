...
class IriTest < Minitest::Test
  def test_builds_uri
    assert_equal(
      'https://localhost:8080/?q=books+about+tennis&limit=10',
      Iri.new('http://google.com/')
  ...
  def test_converts_to_uri
    assert_equal('https://alpha.local?t=1', Iri.new('https://alpha.local').over(t: 1).to_uri.to_s)
  end

  def test_hides_https_port_number
    assert_equal('https://google.com/abc', Iri.new('https://google.com').port(443).append('abc').to_s)
  end
  ...
  def test_starts_with_empty_uri
    assert_equal('https:', Iri.new.scheme('https').to_s)
  end

  def test_inspects_iri
    assert_equal('"https://openai.com"', Iri.new('https://openai.com').inspect)
  end

  def test_replaces_scheme
    assert_equal('https://google.com/', Iri.new('http://google.com/').scheme('https').to_s)
  end

  def test_replaces_host
    assert_equal('http://localhost/', Iri.new('http://google.com/').host('localhost').to_s)
  end

  def test_replaces_port
    assert_equal('http://localhost:443/', Iri.new('http://localhost/').port(443).to_s)
  end

  def test_host_with_no_scheme
    assert_equal('http://localhost/hi', Iri.new.host('localhost').append('hi').to_s)
  end

  def test_port_with_no_scheme
    assert_equal('http://localhost:8080/', Iri.new('//localhost/').port(8080).to_s)
  end

  def test_replaces_fragment
    assert_equal('http://localhost/a/b#test%20me', Iri.new('http://localhost/a/b#before').fragment('test me').to_s)
    assert_equal('http://localhost/#42', Iri.new('http://localhost/').fragment(42).to_s)
  end
  ...
  def test_sets_path
    assert_equal('http://localhost/hey/you?i=8#test', Iri.new('http://localhost/hey?i=8#test').path('/hey/you').to_s)
  end

  def test_sets_query
    assert_equal('http://localhost/hey?t=1#test', Iri.new('http://localhost/hey?i=8#test').query('t=1').to_s)
  end
  ...
  def test_removes_path
    assert_equal('http://localhost/', Iri.new('http://localhost/hey?i=8#test').cut.to_s)
  end

  def test_replaces_everything
    assert_equal('http://localhost/boom-11', Iri.new('http://localhost/hey?i=8#ops').cut('/boom-11').to_s)
  end
  ...
  def test_adds_query_param
    assert_equal('http://google/?a=1&a=3&b=2', Iri.new('http://google/').add(a: 1, b: 2).add(a: 3).to_s)
    assert_equal(
  ...
  def test_removes_query_param
    assert_equal('http://google/?b=2&c=3', Iri.new('http://google/?a=1&b=2&c=3&a=3').del('a').del('x').to_s)
  end
  ...
  def test_appends_to_empty_ending
    assert_equal('http://google.com/hello', Iri.new('http://google.com/').append('hello').to_s)
  end

  def test_appends_empty_path
    assert_equal('http://google.com/test', Iri.new('http://google.com/hello').cut.append('test').to_s)
  end

  def test_replaces_query_param
    assert_equal('http://google/?a=hey&b=2&c=3', Iri.new('http://google/?a=1&b=2&c=3&a=33').over(a: 'hey').to_s)
  end
  ...
  def test_makes_uri_global
    assert_equal('https://google.com/hello', Iri.new('/hello').to_local.scheme('https').host('google.com').to_s)
  end
  ...
  def test_adds_nio
    assert_equal('http://google.com/?b=10', Iri.new('http://google.com/').add(a: nil, b: 10).to_s)
  end

  def test_with_alias_for_add
    assert_equal('http://google.com?q=test&limit=10', Iri.new('http://google.com').with(q: 'test', limit: 10).to_s)
  end
